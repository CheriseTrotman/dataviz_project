...
tabyl(pets$animal_s_name)
##How many different species are included in this dataset (species)?
pets %>% group_by(species) %>% summarise(count=n())
...
steak %>% filter(Do.you.eat.steak.=="Yes") %>% summarise(count=n())
## How many different (unique) responses were there to the question “How do you like your steak prepared?
steak %>% group_by(How.do.you.like.your.steak.prepared.) %>% summarise(count=n())
...
ggsave(path = "figures/explanatory_figures", plot=p, filename = "steak_R.png", width =8, height = 4)
```
...
p1 <- ggplot(pref) +
  ## specify you want to generate a bar chart
  geom_bar(aes(x = steak_pref, y = n, fill = steak_pref),
                stat = 'identity', 
                width = 0.7) +
  ## change the colors of the bars
  scale_color_manual(values = c("#370B01", 
...
## save plot to figures/explanatory_figures directory
ggsave(path = "figures/explanatory_figures", plot = p1, filename = "steak_1.png", width =8, height = 4)
```
